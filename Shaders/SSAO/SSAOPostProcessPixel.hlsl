...
Texture2D deferredPass : register(t2);
...
float4 PsMain(PSInput input) : SV_Target
{
    ...
    float3 deferredPixelColor = float3(0.0f, 0.0f, 0.0f);

    for (int i = 0; i < 9; i++)
    {
        bloomPixelColor += bloomTexture.Sample(textureSampler, input.texCoord) * blurKernel[i];
        deferredPixelColor += deferredPass.Sample(textureSampler, input.texCoord) * blurKernel[i];

    }
    const float GAMMA = 2.2f;
    const float3 GAMMA_CORRECTION = float3(1.0f / GAMMA, 1.0f / GAMMA, 1.0f / GAMMA);
    
    float3 hdrColor = renderTexture.Sample(textureSampler, input.texCoord).xyz + bloomPixelColor + deferredPixelColor;
